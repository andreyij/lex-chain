 ...
 * Do not use this class directly, use te singleton class WordNetHelper
 * @author Andrei
 *
 */
public class WordNet {
    ...
    {
    	Synset s1 = rel.getSourceSynset();//sensul primului cuvant
    	Synset s2 = rel.getTargetSynset();//sensul celui de-al doilea cuvant
    ...
    public Synset intersect(PointerTargetNodeList list1,PointerTargetNodeList list2)
    {
    	for (int i = 0; i < list1.size(); i++)
    	{
    ...
    public PointerTargetNodeList expandSynset(Synset s) throws JWNLException
    {
		...
		// Decomendare pentru a tine cont de cat mai multe relatii
    ...
    public RelParent areRelated(Synset start,Synset end) throws JWNLException
    {
    	...
    	{
    		Synset s = intersect(relatedList1, relatedList2); 
    		if (s != null)
    		{

    			return new RelParent(s,i+1);
    		}
    		if (i < 1)
    		{
    ...
    public  RelParent getShortestPathLCA(IndexWord word1, IndexWord word2) throws JWNLException
    {
        ...
        Synset[] endSenses = word2.getSenses();
        int min = 99;
        RelParent lca = new RelParent(null,100);
        for (int i = 0; i < startSenses.length; i++) {
            for (int j = 0; j < endSenses.length; j++) {
            	RelParent rel = areRelated(startSenses[i], endSenses[j]); 
