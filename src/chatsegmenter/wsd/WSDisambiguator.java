...
public class WSDisambiguator {
	...
	private  Vector<UttTaggedWord> utt;
	private Vector<Vector<Integer>> valid;
	public WSDisambiguator(Vector<UttTaggedWord> utt) throws JWNLException
	{
	...
	public  Vector<UttTaggedSenseWord> wsd()
	{
		if (LexChain.DEBUG_PRINT)
			System.out.println("here wsd");
		
		back(0);
		Vector<UttTaggedSenseWord> sdWords = new Vector<UttTaggedSenseWord>();
		...
		for (int i = 0; i < nrOfWords; i++)
		{
			...
			sdWords.add(new UttTaggedSenseWord(utt.elementAt(i),sense)); 
		}
		if (LexChain.DEBUG_PRINT)
